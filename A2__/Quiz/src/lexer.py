    ...
    while i < len(s): # Loop through the input string
        if s.isspace() or s[i] == '\n': # If the character is a space or a newline, skip it
            ...
            if is_decimal: # If the number is a decimal number
                tokens.append(DecimalNumberToken(dig)) # Append the token to the list of tokens
                assert i == len(s) or s[i].isspace() or s[i] == '\n', "INVALID NUMBER" # Assert that the character is a space or a newline
            else: # If the number is not a decimal number
                tokens.append(NumberToken(dig)) # Append the token to the list of tokens
                assert i == len(s) or s[i].isspace() or s[i] == '\n', "INVALID NUMBER" # Assert that the character is a space or a newline
            ...
            while i < len(s) and s[i].isdigit() or s[i] == '.': # Loop through the input string while the character is a digit or a decimal point
                ...
                i += 1 # Increment the index
            if is_decimal: # If the number is a decimal number
                tokens.append(NegDecimalNumberToken(dig)) # Append the token to the list of tokens
                assert i == len(s) or s[i].isspace() or s[i] == '\n', "INVALID NUMBER" # Assert that the character is a space or a newline
            else: # If the number is not a decimal number
                tokens.append(NegNumberToken(dig)) # Append the token to the list of tokens
                assert i == len(s) or s[i].isspace() or s[i] == '\n', "INVALID NUMBER" # Assert that the character is a space or a newline
            ...
            while i < len(s) and s[i] != '\n': # Loop through the input string while the character is not a double quote or a newline
                ...
                i += 1 # Increment the index
            if i == len(s)-1 and s[i] != '"': # If the character is the last character and it is not a double quote, raise an exception
                raise Exception("Invalid string") # Raise an exception
            assert s[i] == '"', "INVALID STRING" # Assert that the character is a double quote
            ...
            assert i == len(s) or s[i].isspace() or s[i] == '\n', "INVALID WORD" # Assert that the character is a space or a newline or the end of the string
            i+=1 # Increment the index
